(() => {
 ...
 function install(){
  for(const id of ['emailFormView','gcFormView']){
   ...
   const heading=host.querySelector('h3');if(heading)heading.textContent='Text My Coupon on WhatsApp';
   const intro=heading?.nextElementSibling;if(intro?.tagName==='P')intro.hidden=true;
   ...
   const note=document.createElement('p');note.textContent='Join my deals group, then tap “I’ve joined” in WhatsApp.';
   ...
   section.append(button,note,status);form.before(section);
  }
 }
