(() => {
  ...
  function render(){
    ...
    for(const r of rows){
      ...
      card.append(node('div',r.viewers?pct(r.conversion_rate):'—','rate'),node('p','Coupon-send conversion','note'));
      card.append(node('p',r.sent+' sent / '+r.viewers+' popup viewers','note'));
      const bar=node('div',undefined,'bar'),fill=node('span');fill.style.width=pct(r.conversion_rate);bar.append(fill);card.append(bar);
      card.append(node('p',r.clicks+' clicked · '+r.no_click+' have not clicked','note'));
      const link=node('a','Preview '+r.variant+' (not tracked)','preview');link.href='/zernio-preview.html?zernio_preview='+r.variant;link.target='_blank';link.rel='noopener noreferrer';card.append(link);el('variants').append(card);
      const row=node('tr');for(const value of [r.variant,r.viewers,r.clicks,r.no_click,r.dismissals,r.exits,r.requests,r.messages,r.confirmed,r.sent,r.viewers?pct(r.click_rate):'—',r.viewers?pct(r.conversion_rate):'—',r.viewers?r.interval.map(pct).join(' – '):'—'])row.append(node('td',String(value)));el('funnel').append(row);
    }
  ...
  async function load(){
    ...
    try{const r=await fetch('https://greatclips-email.mehulchaudhari.workers.dev/zernio/experiment/report',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token},body:'{}'});const result=await r.json();if(!r.ok||result.error)throw new Error(result.error||'Unable to load results.');data=result;render();}
  ...
  el('download').onclick=()=>{if(!data)return;const keys=['variant','viewers','clicks','no_click','dismissals','exits','requests','messages','confirmed','sent','click_rate','conversion_rate'];const rows=el('mature').checked?data.mature:data.live;const csv=[keys.join(','),...rows.map(r=>keys.map(k=>r[k]).join(','))].join('\r\n');const url=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));const a=document.createElement('a');a.href=url;a.download=data.experiment_id+(el('mature').checked?'-mature':'-live')+'.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
})();
